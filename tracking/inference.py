...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        self.particles=[]
        temp = self.numParticles
        while temp > 0:
            self.particles.extend(self.legalPositions)
            temp -= len(self.legalPositions)
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            jail = self.getJailPosition()
            allPossible[jail] = 1
            self.particles = allPossible #case 1/ pacman captured a ghost
            ...
            for pos in self.legalPositions:  
                ...
                allPossible[pos] = emissionModel[trueDistance] * beliefs[pos] #add the weights

            if allPossible.totalCount() == 0: ##case 2 all particles recieve weight 0
                self.initializeUniformly(gameState)

            else:
                self.particles = []
                allPossible.normalize() #normalize the weights

                #resampling the particles
                for i in range(self.numParticles): 
                    self.particles.append(util.sample(allPossible))  #select randomly from resulting distribution   
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            newParticle=random.choice(newPosDist.keys()) #sample randomly from new distribution
            tempParticles.append(newParticle) #"replacing" old particle with newParticle
...
class JointParticleFilter:
    ...
    def observeState(self, gameState):
        ...
        for state in self.particles:
            ...
            for i in range(self.numGhosts):
                if noisyDistances[i] != None:
                    trueDistance = util.manhattanDistance(state[i], pacmanPosition)
                    productOfWeights *= emissionModels[i][trueDistance]
            allPossible[state] += productOfWeights
